...
import './style.scss'
import {ReactComponent as AvailableTruckIcon} from './svg/AvailableTruck.svg'
import {ReactComponent as OnMarketTruckIcon} from './svg/OnMarketTruck.svg';
...
export default function CreateTrip() {
    ...
    let HandleCreateTrip = async () => {
        ...
        try{
            ...
            if(CreateTripResponse && CreateTripResponse.success){
                ...
                await setTripId(CreateTripResponse.trip_id);
                setTripPage(2);
            }
            else{
    ...
    return (
        <>
            ...
            <div className="px-lg-3 px-md-2 px-1 mt-5 create-trip-container">
                    ...
                    <form className="w-100 d-flex flex-column">
                        ...
                        <div className="d-flex justify-content-between align-items-center">
                            ...
                            >
                                ...
                                <div className="w-100 available-truck-list">

                                    {trucks.map(truck => 
                                         <MenuItem className="mt-2">{
                                            <div onClick={() => setTripDetails({...tripDetails,truckNumber: truck.truck_number})} className="d-flex justify-content-between align-items-center available-trucks-list">
                                                <span className="col-4 overflow-scroll">{truck.truck_number}</span>
                                                <Icon className="col-3">{truck.is_available ? <AvailableTruckIcon /> : <OnMarketTruckIcon />}</Icon>
                                                <p className={"col-4" + (truck.is_available ? " text-success": " text-danger")}>{truck.is_available ? "Available" : "On Market"}</p>
                                            </div>
                                        }</MenuItem>
                                    )}
                                </div>
                ...
                {editTrip===false && <Accordion className="mt-4">
                    ...
                    <AccordionDetails className="d-flex flex-column create-trip-accordian-details">
                        <div className="w-100 d-flex justify-content-between align-items-end add-billing-details">
                            <div className="d-flex justify-content-evenly align-items-end flex-wrap">
                                ...
                                <Button className={tripDetails.billingType === billingTypes[0] ? "active" : ""} variant={tripDetails.billingType === billingTypes[0] ? "contained" : "outlined"} onClick={() => BillingTypeChange(0)} endIcon={tripDetails.billingType === billingTypes[0] && <CheckOutlined />}>{billingTypes[0].replace('-',' ')}</Button>
                                <Button className={tripDetails.billingType === billingTypes[1] ? "active" : ""} variant={tripDetails.billingType === billingTypes[1] ? "contained" : "outlined"} onClick={() => BillingTypeChange(1)} endIcon={tripDetails.billingType === billingTypes[1] && <CheckOutlined />}>{billingTypes[1].replace('-',' ')}</Button>
                                <Button className={tripDetails.billingType === billingTypes[2] ? "active" : ""} variant={tripDetails.billingType === billingTypes[2] ? "contained" : "outlined"} onClick={() => BillingTypeChange(2)} endIcon={tripDetails.billingType === billingTypes[2] && <CheckOutlined />}>{billingTypes[2].replace('-',' ')}</Button>
                                <Button className={tripDetails.billingType === billingTypes[3] ? "active" : ""} variant={tripDetails.billingType === billingTypes[3] ? "contained" : "outlined"} onClick={() => BillingTypeChange(3)} endIcon={tripDetails.billingType === billingTypes[3] && <CheckOutlined />}>{billingTypes[3].replace('-',' ')}</Button>
                            </div>   
