...
import {ReactComponent as MiniTruckIcon} from './svg/MiniTruck.svg'
import {ReactComponent as OpenBodyTruckIcon} from './svg/OpenBodyTruck.svg'
import {ReactComponent as ClosedContainerTruckIcon} from './svg/ClosedContainerTruck.svg'
import {ReactComponent as TrailerTruckIcon} from './svg/TrailerTruck.svg'
import './style.scss'
...
export default function AddTruckModal(props) {
    ...
    return (
        <Dialog fullWidth open={props.open} hideBackdrop disableBackdropClick onClose={props.close} maxWidth="lg">
            <form className="d-flex justify-content-between align-items-center w-100 px-lg-3 px-md-2 px-0 py-2 add-truck-form">
                <div className="col-lg-6 col-md-6 col-11 mx-auto">
                    <h3 className="mt-3">Add Truck</h3>
                    <h6 className="mt-3">Select a truck Type</h6>

                    <RadioGroup className="d-flex flex-column my-2 truck-types" value={truckDetails.containerType} onChange={(e) => setTruckDetails({...truckDetails,containerType: e.target.value})}>
                        {truckTypes.length<=0 ? <PulseLoader size={15} margin={2} color="#36D7B7" /> : truckTypes.map((truck,i) => 
                            <div className={`d-flex px-2 justify-content-between align-items-center py-1 ${truck.toString().replaceAll('/','-').replaceAll(' ','-').toLowerCase()}`}>
                ...
                <div className="col-lg-6 col-md-6 col-11 mx-auto px-lg-3 px-md-2 px-0">
                    ...
                    />
                    <TextField 
                        className="col-lg-8 col-md-8 col-11 mt-3 mx-auto"
                        label="Driver Name (optional)"
                        value={truckDetails.customerName}
                        onChange={(e) => setTruckDetails({...truckDetails,customerName: e.target.value})}
                    />
